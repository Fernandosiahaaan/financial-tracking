...
public class HandlerException {
        ...
        public ResponseEntity<BaseResponse> handleNotFound(NotFoundException ex) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                                .body(BaseResponse.setResponse(false, ex.getMessage(), null, ex));
        }
        ...
        public ResponseEntity<BaseResponse> handleValidationException(MethodArgumentNotValidException ex) {
                ...
                return ResponseEntity.badRequest()
                                .body(BaseResponse.setResponse(false,
                                                "kesalahan pada sistem, mohon menghubungi petugas [E002]",
                                                "[ValidationException] " + messageErr, null));
        }

        @ExceptionHandler(BadRequestException.class)
        public ResponseEntity<BaseResponse> handleBadRequest(BadRequestException ex) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
                                BaseResponse.setResponse(false, ex.getMessage(),
                                                "[BadRequestException] " + ex.getMessage(), null));
        }
        ...
        public ResponseEntity<BaseResponse> handleRunTime(RuntimeException ex) {
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                .body(BaseResponse.setResponse(false,
                                                "kesalahan pada sistem, mohon menghubungi petugas [E004]",
                                                "[INTERNAL_SERVER_ERROR] : " + ex.getMessage(), null));
        }
